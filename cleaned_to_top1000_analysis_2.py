...
# brand_code_counts = df_cleaned.groupby("Brand")["Product Code"].nunique()
...
#     Unique_Product_Codes=('Product Code', 'nunique'),
#     Total_Products=('Brand', 'count')
# ).reset_index()

# df_cleaned["Product Name"] = df_cleaned["Product Name"].astype(str).str.lower()
...
# mask = df_cleaned["Product Name"].apply(lambda x: any(keyword in x for keyword in unwanted_keywords))
...
# df_cleaned["Gender Category"] = df_cleaned["Product Name"].apply(categorize_gender)
# print(df_cleaned["Gender Category"].value_counts())

# df_cleaned["Product Price"] = df_cleaned["Product Price"].astype(str)
# df_cleaned["Product Price"] = df_cleaned["Product Price"].str.replace("₹", "", regex=False)
# df_cleaned["Product Price"] = df_cleaned["Product Price"].str.replace(",", "", regex=False).str.strip()

# # Extract only the first valid numeric price (between 3 and 6 digits)
# df_cleaned["Product Price"] = df_cleaned["Product Price"].str.extract(r'(\d{3,6})')

# # Convert to numeric safely
# df_cleaned["Product Price"] = pd.to_numeric(df_cleaned["Product Price"], errors="coerce")

# ##SKUs

# # Step 1: Clean the 'Product Price' column
# df_cleaned["Product Price"] = (
#     df_cleaned["Product Price"]
#     .astype(str)
...
# ]
# df_cleaned["Price Bin"] = pd.cut(df_cleaned["Product Price"], bins=bins, labels=labels, right=False)
...
# df_men = df_filtered[df_filtered["Gender Category"] == "Men"]
# df_women = df_filtered[df_filtered["Gender Category"] == "Women"]

# # Step 5: Group and pivot
# sku_table_men = df_men.groupby(["Price Bin", "Brand"]).size().unstack(fill_value=0).reindex(labels)
# sku_table_women = df_women.groupby(["Price Bin", "Brand"]).size().unstack(fill_value=0).reindex(labels)
...
# df_cleaned["Price Range"] = df_cleaned["Product Price"].apply(categorize_price)

# df_cleaned = df_cleaned[df_cleaned["Price Range"] != "Unknown"].copy()

# # Optional: Reset index
# df_cleaned.reset_index(drop=True, inplace=True)

# # Confirm it's dropped
# print(df_cleaned["Price Range"].value_counts())
...
# df_cleaned.rename(columns={"Product Code": "SKUs"}, inplace=True)

# # 1. Products listed by Brand, Price Range, Gender Category
# product_count = df_cleaned.groupby(["Brand", "Price Range", "Gender Category"])["Product Name"].count().reset_index()
# product_count.columns = ["Brand", "Price Range", "Gender Category", "Product Count"]

# # 2. Unique product codes listed by Brand, Price Range, Gender Category
# unique_code_count = df_cleaned.groupby(["Brand", "Price Range", "Gender Category"])["SKUs"].nunique().reset_index()
# unique_code_count.columns = ["Brand", "Price Range", "Gender Category", "SKUs"]

# # Pivot Men tables
# men_product_table = product_count[product_count["Gender Category"] == "Men"].pivot_table(
#     index="Brand", columns="Price Range", values="Product Count", fill_value=0).reset_index()
# men_product_table = men_product_table.astype({col: 'int' for col in men_product_table.columns if col != "Brand"})

# men_code_table = unique_code_count[unique_code_count["Gender Category"] == "Men"].pivot_table(
#     index="Brand", columns="Price Range", values="SKUs", fill_value=0).reset_index()
# men_code_table = men_code_table.astype({col: 'int' for col in men_code_table.columns if col != "Brand"})

# # Pivot Women tables
# women_product_table = product_count[product_count["Gender Category"] == "Women"].pivot_table(
#     index="Brand", columns="Price Range", values="Product Count", fill_value=0).reset_index()
# women_product_table = women_product_table.astype({col: 'int' for col in women_product_table.columns if col != "Brand"})

# women_code_table = unique_code_count[unique_code_count["Gender Category"] == "Women"].pivot_table(
#     index="Brand", columns="Price Range", values="SKUs", fill_value=0).reset_index()
...
# brand_product_total = df_cleaned.groupby("Brand")["Product Name"].count().reset_index()
...
# top_1000_product_count = top_1000.groupby("Brand")["Product Name"].count().reset_index()
...
# top_men = top_1000[top_1000["Gender Category"] == "Men"]
# top_women = top_1000[top_1000["Gender Category"] == "Women"]

# # Step 3: Group and count for Men
# men_product_count = top_men.groupby("Brand")["Product Name"].count().reset_index()
...
# women_product_count = top_women.groupby("Brand")["Product Name"].count().reset_index()
